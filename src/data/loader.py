...
class AISDataLoader:
    ...
    def __init__(self, config_obj=None):
        ...
        self.required_columns = [
            'MMSI', 'LAT', 'LON', 'SOG', 'COG', 'VesselType',
            'BaseDateTime', 'Length', 'Width'
        ]
    ...
    def create_sample_data(self, input_path: Union[str, Path], 
                          output_path: Union[str, Path], 
                          sample_size: float = None) -> Path:
        """
    ...
    def basic_data_validation(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df[
            (df['LAT'] >= self.config.MIN_LAT) & (df['LAT'] <= self.config.MAX_LAT) &
            (df['LON'] >= self.config.MIN_LON) & (df['LON'] <= self.config.MAX_LON)
        ]
        ...
        final_count = len(df)
        logger.info(f"Validation complete: {final_count} records ({final_count/initial_count*100:.1f}% retained)")
        
        return df
    
    def load_and_validate(self, filepath: Union[str, Path], 
                         test_mode: bool = None) -> pd.DataFrame:
        """
