...
class AISPreprocessor:
    ...
    def reduce_vessel_types(self, df: pd.DataFrame, 
                           target_types: List[float] = None) -> pd.DataFrame:
        """
    ...
    def process_trajectories(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not df.empty:
            ...
            logger.info(f"Trajectories per vessel - Mean: {traj_stats.mean():.1f}, "
    ...
    def remove_outliers(self, df: pd.DataFrame, 
        ...
        logger.info(f"Outlier removal complete: {removed_total} records removed "
