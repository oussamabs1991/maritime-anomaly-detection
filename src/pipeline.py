...
class MaritimePipeline:
    ...
    def load_and_prepare_data(self, zip_file_path: str = None, 
                             csv_file_name: str = None) -> pd.DataFrame:
        """
    ...
    def prepare_sequence_data(self, df: pd.DataFrame, 
        ...
        for _, row in feature_trajectories.iterrows():
            traj_data = df[
                (df['MMSI'] == row['MMSI']) & (df['traj_id'] == row['traj_id'])
            ]
    ...
    def evaluate_model(self, X_test: np.ndarray, y_test: np.ndarray,
                      seq_test: np.ndarray, class_names: list = None) -> Dict[str, Any]:
        """
    ...
    def create_visualizations(self, y_test: np.ndarray, predictions: np.ndarray,
    ...
    def run_complete_pipeline(self, zip_file_path: str, csv_file_name: str) -> Dict[str, Any]:
        ...
        logger.info(f"Starting complete pipeline in {'TEST' if self.config.TEST_MODE else 'FULL'} mode")
        ...
        (X_train, X_test, y_train, y_test, 
         seq_train, seq_test, class_weights) = self.split_and_scale_data(X, y, sequences)
