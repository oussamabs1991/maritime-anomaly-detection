...
Examples:
  ...
  python main.py --load-model models/ensemble_model.joblib --zip-file data/test.zip --csv-name test.csv
    ...
    parser.add_argument(
        "--zip-file",
        type=str,
        help="Path to ZIP file containing AIS data"
    )
    
    parser.add_argument(
        "--csv-name",
        type=str,
        help="Name of CSV file inside the ZIP"
    )
    ...
    parser.add_argument(
        "--smoke-tests-only",
        action="store_true",
        help="Run smoke tests only and exit"
    )
    ...
    try:
        ...
        if args.load_model:
            ...
            if args.zip_file and args.csv_name:
                ...
                # Evaluate
                class_names = list(pipeline.label_encoder.classes_) if hasattr(pipeline.label_encoder, 'classes_') else None
